...
![TeleZX Page](https://github.com/tebl/telezx/blob/main/pictures/page.png?raw=true)

In order to make this a reality we'll need some content to display, something that I've chosen to separate out into its own separate [Github-repository](https://github.com/tebl/telezx-content). This repository will be referenced to the software components as a "*document repository*". A document has an ID attached to it, and this is how you'll be accessing content though instead up to 999 pages the system uses a 4-digit hex number. Each document can have up to 99 pages attached to it, and as it's intended for the ZX Spectrum we'll be working with either SCR-files or a custom format based on SpecSCII for a more familiar blocky format. 
...
Repository includes an extensive amount of Python-scripts for creating and working with *TeleZX document repositories*. Additionally, as there're a separate document format designed for editing pages I naturally needed an editor in order to actually do so.

![TeleZX Editor](https://github.com/tebl/telezx/blob/main/pictures/editor.png?raw=true)
...
![TeleZX webclient](https://github.com/tebl/telezx/blob/main/pictures/webclient.png?raw=true)
